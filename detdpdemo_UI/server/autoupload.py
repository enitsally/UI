...
class detdpautoupload:
  ...
  def get_file(self):
    ...
    if data_prefix is not None and conf_prefix is not None:
      dfileList = [file.lower() for file in filenames if file.endswith(suffix) and file.startswith(data_prefix)]
      cfileList = [file.lower() for file in filenames if file.endswith(suffix) and file.startswith(conf_prefix)]
    else:
        ...
        with open(path + conf_name, 'rb') as conf_file:
          ...
          if log_dict[key]['ready_upload'] == 'Y':
            ...
            if len(link_list) > 0:
              ...
              log_dict[key]['column_check'] = 'System Failed'
              sys_mgs += '''No link cols exist in database, need administration setup. '''

            if len(std_col) > 0 and log_dict[key]['column_check'] == 'Check Pass':
              for col in std_col:
